...
import {
    driveReadText, driveWriteFile, driveFindFile, stockFileByName,
    STOCK_FOLDER_ID, SITE_CONFIG_FILENAME,
} from '../lib/google.js';
import {
    ...
    if (!verifyToken('admin', bearer(req))) {
      ...
      if (!Array.isArray(body.config.categories)) {
              return fail(req, 400, 'Refusing to save: config has no categories array');
                                 ...
                                 await driveWriteFile(fileId, JSON.stringify(body.config, null, 2));
